...
for game in tqdm(train_games):
    for item in my_data.get_boards(game):
        ...
        nx_graph = my_graph.chess_nx(board)
        graph = from_networkx(nx_graph)
        graph.x = torch.tensor([[node[1]['piece_value'], node[1]['piece_color'], node[1]['piece_type'], node[1]['attackers_defenders']] for node in nx_graph.nodes(data=True)], dtype=torch.float).view(-1, 4)
        graph.y = win
...
class GCN(torch.nn.Module):
    def __init__(self, n_node_features=4, hidden_channels=CONFIG['hidden_channels'], n_cls=2):
